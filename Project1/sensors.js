...
class Sensors {
   ...
   */
    ...
    async addSensorType(info) {
        ...
        this.stype.set(sensorType.id, sensorType);
   ...
   */
    async addSensor(info) {
        ...
        this.ssensor.set(sensor.id, sensor);
    }
   ...
   */
    async addSensorData(info) {
        ...
        if(this.sdata.has(sensorData.sensorId))
        {
            let val = this.sdata.get(sensorData.sensorId);
            val.push(sensorData);
        }
        else{
            let val = new Array(sensorData);
            this.sdata.set(sensorData.sensorId, val);
        }
    }
   ...
   */
    ...
    async findSensorTypes(info) {
        ...
        let id = searchSpecs.id;
        let index = searchSpecs.index;
        let count = searchSpecs.count;
        let result = [];
        let filters = {};
        let new_stype = new Map([...this.stype.entries()].sort());
        for(let k in searchSpecs)
        {
            if(k==="id" || k==="index" || k==="count")
            {
                continue;
            }
            else{
        ...
        if(id != null)
        {
            let element = new_stype.get(id);
            for(let k in filters)
                if(element[k] != filters[k])
                    bool = 0;
            if(element != null && bool===1)
                result.push(element);
        }
        else
        {
            try {
                    new_stype.forEach(function (element, key) {
                    if (count === 0)
                        throw new Error();
                    if (indexCount >= index) {
                        bool = 1;
   ...
   */
    ...
    async findSensors(info) {
        ...
        let index = searchSpecs.index;
        let count = searchSpecs.count;
        let dodetail = searchSpecs.doDetail;
        let result = [];
        let filters = {};
        let new_ssensor = new Map([...this.ssensor.entries()].sort());
        for (let k in searchSpecs) {
            if (k === "id" || k === "index" || k === "count") {
                continue;
            } else {
        ...
        let bool = 1;
        if (id != null) {
            let element = new_ssensor.get(id);
            for (let k in filters)
                if (element[k] != filters[k])
                    bool = 0;
            if (element != null && bool === 1)
            ...
            try {
                    new_ssensor.forEach(function (element, key) {
                    if (count === 0)
                        throw new Error();
                    if (indexCount >= index) {
                        bool = 1;
        ...
        if (indexCount >= new_ssensor.size)
            indexCount = -1;
        let resultWrapper = {
            "nextIndex": indexCount,
            "data": result
   ...
   */
    async findSensorData(info) {
        ...
        if(dodetail==="true")
        {
            let sensor = this.ssensor.get(id);
            let type = this.stype.get(sensor["model"]);
            resultWrapper["sensorType"] = type;
...
/*function sortCallback(entry1, entry2){
    return entry2[1]["timestamp"] - entry1[1]["timestamp"];
}*/
